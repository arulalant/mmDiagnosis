...
##     x.setcolormap('cmap')
    ...
    tmpl.reset('y',.2,.85,tmpl.data.y1,tmpl.data.y2)
    ...
    tmplnoleg.yname.priority=0
    tmplnoleg.xname.priority=0
    
    isof=x.createisofill()
    levs2=list(numpy.arange(min,max,deltaisof))
#    levs2=list(numpy.arange(1.,2.1,.1))
    if min >= 0:
        levs1a = [0]
    else:
    ...
    isof.levels=levs2
#    print "levels ", levs2, "len ", len(levs2)
    c = vcs.getcolors(levs2,colors=range(16,40))
#    dummy = [240]*11
    c =  [240, 6, 47, 246, 255, 111, 143, 245, 175, 242, 239]

    isof.fillareacolors= c #(vcs.getcolors(levs2,colors=range(16,40),split=2))
