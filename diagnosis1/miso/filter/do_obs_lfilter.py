...
_weightPath = os.path.join(__curDir__, 'LanczosFilter_weights_25_90_days.dat')
...
def applyLanczosFilter(rawOrAnomaly, filteredOrNot, weights=lweights, **kwarg):
    ...
    inpath = os.path.join(processfilesPath, 'Anomaly', 'Unfiltered')
    if suffixPath:
    ...
    for subName in os.listdir(inpath):
        ...
        outpath = os.path.join(processfilesPath,
                          'Anomaly', 'Filtered', 'miso')
        if suffixPath:
        ...
        for anofile in os.listdir(anopath):
            ...
            outf.close()
            log.info("Created lfiltered file '%s'", outfile)
            # make memory free
            del filteredAnomaly
...
if __name__ == '__main__':
    ...
    for climatology in climatologies:
        if not climatolgy.name.lower() == 'miso': continue
        for obs in observations:
            if not obs.name.upper() == 'GPCP': continue
            log.info("The Lanczos Filter has started for the obs '%s'", obs.name)
            obsPathList = [obs.name, climatology.name]
            applyLanczosFilter('Anomaly', 'Unfiltered',
                    suffixpath=obsPathList, overwrite=True, log=log)
            log.info("The Lanczos Filter has finished for the obs '%s'", obs.name)
